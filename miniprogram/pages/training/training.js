...
<<<<<<< HEAD
  ...
  handleWSMessage(data) {
    switch (data.type) {
      ...
      case 'analysis':
        const score = data.score != null ? Math.round(data.score) : this.data.currentScore;
        ...
        if (data.count != null && data.count !== totalCount) {
          totalCount = data.count;
          validCount = data.valid_count != null ? data.valid_count : validCount;
          errorCount = data.error_count != null ? data.error_count : errorCount;
        }
        ...
        if (score > 0) {
          this.data.scoresHistory.push(score);
        }
        break;
  ...
  handleCameraFrame(frame) {
    ...
    const now = Date.now();
    if (now - this._lastFrameTime < this.FRAME_INTERVAL) return;
...
<<<<<<< HEAD
  ...
  _applyDemoFrame(frame, index, total) {
    ...
    const score = frame.score != null ? Math.round(frame.score) : this.data.currentScore;
