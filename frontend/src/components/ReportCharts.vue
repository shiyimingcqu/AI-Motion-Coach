<template>
  <section class="report-charts">
    <article v-if="showTrend" class="chart-card wide">
      <header>
        <h3>{{ t("reportCharts.scoreTrend") }}</h3>
    ...
    <div class="chart-row">
      <article class="chart-card">
        <header>
          <h3>{{ t("reportCharts.calories") }}</h3>
          <span>{{ t("reportCharts.caloriesSubtitle") }}</span>
        </header>
      ...
      <article class="chart-card">
        <header>
          <h3>{{ t("reportCharts.distribution") }}</h3>
          <span>{{ t("reportCharts.distributionSubtitle") }}</span>
        </header>
    ...
    <div class="chart-row">
      <article class="chart-card">
        <header>
          <h3>{{ t("reportCharts.qualityRadar") }}</h3>
      ...
      <article class="chart-card">
        <header>
          <h3>{{ t("reportCharts.errors") }}</h3>
          <span>{{ t("reportCharts.errorsSubtitle") }}</span>
        </header>
...
const radarSubtitle = computed(() => props.radarSubtitle || t("reportCharts.radarSubtitle"));
const trendSubtitle = computed(() => props.trendSubtitle || t("reportCharts.trendSubtitle"));
...
function buildTrendOption() {
  const seriesList = props.trendSeries?.length
    ? props.trendSeries
    : [{ name: t("reportCharts.comprehensive"), trend: props.charts.score_trend }];
