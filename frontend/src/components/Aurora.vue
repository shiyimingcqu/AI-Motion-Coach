...
onMounted(() => {
  ...
  const initialStops = props.colorStops.map((hex: string) => {
  ...
  const update = (t: number) => {
    ...
    const stops = (p.colorStops ?? props.colorStops).map((hex: string) => {
      const c = new Color(hex);
      return [c.r, c.g, c.b];
    });
